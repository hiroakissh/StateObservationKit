...
- `send(_:)` は同じ順序付き Reducer キュー上で、確定した state を待機します。
- Reducer 実行は内部で逐次化され、`dispatch(_:)` と `send(_:)` の呼び出し順を保ちます。
- 新しい状態は各 Reducer 実行の完了後に MainActor 上で公開されます。
- `dispatch(_:)` は fire-and-forget として扱い、明示的な完了点が必要な場合は `send(_:)` を使います。
