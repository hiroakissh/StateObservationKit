...
- **入力意図 (Intent / current API では Action) は明示的かつ型安全に定義する**
  - `enum` を用いた定義によって、受け付ける入力と遷移先を網羅し、意図しない分岐や既定の fallthrough を排除します。
...
StateObservationKit は、アプリ全体で一貫した状態駆動設計を行うための骨組みを提供します。状態・入力・副作用の責務を明瞭に分離し、テストしやすく拡張性の高いアプリケーションを構築するための共通言語を目指しています。
