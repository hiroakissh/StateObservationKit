...
| `main` | release branch | `develop` からの PR merge のみ受け付け、workflow でも merged PR 由来の push だけを release 対象にします。 |
...
| `main` | release branch | `develop` からの PR merge のみ受け付け、merge 後に `release-tag.yml` が merged PR 由来の commit だけを release tag 化します。 |
...
- `main` merge 時に `VERSION` が前回の `main` と異なる場合、または `VERSION` 導入直後で同名 tag がまだ存在しない場合だけ、新しい annotated tag を作成します。
...
- たとえば `VERSION=1.4.0-beta.1` を `main` へ PR merge すれば、tag `1.4.0-beta.1` が作成されます。
